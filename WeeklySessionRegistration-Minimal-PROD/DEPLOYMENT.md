...
2. Select `Flow/EventSessionFlows_NoTemplates_1_0_0_18.zip`.
3. Map the **four connections** when prompted: SharePoint, Office 365
   ...
   > **Who should own these connections?** Ideally a dedicated service
   > account — whoever owns the Outlook connection is who the emails
   > come *from*. See **`Docs/CONNECTIONS.md`** for the recommended
   > setup, the **Run only users** settings (which, if left on
   > *"Provided by run-only user"*, make emails go out from the
   > registrant), and how to rebind an environment after the fact.
...
| Symptom | Fix |
|---|---|
| Import error "…not declared in the solution file as a root component" | You are importing an old zip; use `EventSessionFlows_NoTemplates_1_0_0_18.zip` from this kit |
...
| Emails come from the wrong person (e.g. the registrant) | Run-only users set to “Provided by run-only user”, or connection references bound to a personal connection — see `Docs/CONNECTIONS.md` |
| Teams card shows literal `\n` | Old flow version — re-import v1.0.0.18 |
